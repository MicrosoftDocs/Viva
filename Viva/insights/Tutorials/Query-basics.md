...
Your tenant's choice of billing model affects the appearance and behavior of the pages for creating queries (such as [person queries](person-queries.md) and [meeting queries](meeting-queries.md)) and the [query results](../use/view-download-and-export-query-results.md) page. For more information, see [consumption-model billing](consump-model.md#for-analysts-in-consumption-model-tenants) and [per-user-per-month model](consump-model.md##for-analysts-in-pupm-model-tenants).
