...
Your tenant's choice of billing model affects the appearance and behavior of the pages for creating and running queries (such as for [person queries](person-queries.md)) and the [query results](../use/view-download-and-export-query-results.md) page. For more information about these billing models, see [consumption-model billing](consump-model.md#for-analysts-in-consumption-model-tenants) and [per-user-per-month billing](consump-model.md##for-analysts-in-pupm-model-tenants).
