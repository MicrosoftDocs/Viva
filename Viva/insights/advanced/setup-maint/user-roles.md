...
|   |  | **Analysis** | Landing page for Analysis. View recent queries, PBI templates and build custom queries.  |   | X |   |   |
|   |  | **Query results** | View query results |   | X |   |   |
|   |  | **Organizational data** | Verify if  organizational data quality is good enough for analysis and upload   custom organizational data file. | X | X<sup>3</sup>|   |   |
