...
The advanced insights app can get organizational data in one of two ways: through Azure Active Directory, which is the default setting, or through an organizational data file that you as an admin upload. Azure Active Directory automatically syncs with the advanced insights app and provides data for the **PersonId**, **ManagerId**, **Organization**, **Domain**, and **Timezone** attributes. Domain will be based on the email address value of the user in Azure Active Directory and Timezone is synced from the Outlook settings. If you want to include more attributes, for example, those we discuss in [Prepare organizational data](prepare-org-data.md#attribute-reference), then you'd want to choose a data file as your source.
