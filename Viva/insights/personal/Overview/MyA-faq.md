...
title: FAQ for Personal Viva Insights
description: Frequently asked questions about personal insights in Microsoft Viva Insights
author: madehmer
ms.author: v-pausch
ms.topic: article
...
The questions in this topic are organized into the following sections about personal insights in Microsoft Viva Insights (formerly MyAnalytics):

* The [Privacy](#privacy) section applies to everyone.

The other questions and answers are grouped by role:

* For [Viva Insights users](#for-viva-insights-users)
...
Only you can see your data. The statistics and insights that are generated from your data are for your eyes only. Your manager or system administrator cannot see your personal data. For details, see the [Personal insights privacy guide](privacy-guide-users.md).
...
## For Viva Insights users

### Data sources

#### Q1. Can data be extracted from on-premises installations of Microsoft Exchange or Skype for Business?

No. Only Exchange Online, Skype for Business Online, and Teams are used as sources of Viva Insights data.
...
#### Q1. How do the Viva Insights digests show up in Outlook?

 Your [Viva Insights digests](../use/email-digest-2.md) are system generated notifications that do not go through the standard email delivery process. They are inserted directly into your Outlook inbox by Viva Insights.

### Opt out

#### Q1. Can I opt out of Viva Insights? And if I do (or if an admin opts me out), can I opt back in later?

Yes to both questions. You can opt out of access to individual parts of Viva Insights or out of all of Viva Insights. And you can [opt back in](../use/opt-out-of-mya.md#if-i-opt-out-can-i-opt-back-in) again later, if you want.
...
#### Opt out of individual parts of Viva Insights

* [Opt out of the dashboard](../use/dashboard-2.md#opt-out-of-the-dashboard)
* [Opt out of the email digests](../use/email-digest-2.md#opt-out-of-digests)
* [Opt out of the Outlook add-in](../use/add-in.md#to-opt-out)
...
* **Opt out** - If you opt out, you lose access to the feature. (But remember that you can opt back in if you change your mind. To do so, follow the steps in [Opt out of the Insights Outlook add-in](../use/add-in.md#to-opt-out) but in step 4, set the control to **On**.)
* **Remove** - If you [remove the Insights Outlook add-in](#remove-the-insights-outlook-add-in), not only do you lose access to the feature, its icon is also removed from your Outlook ribbon. Note that you can change your mind about this. See [Add the Insights Outlook add-in](#add-the-insights-outlook-add-in).  
...
The Viva Insights dashboard is only available if your organization has a *qualifying plan*. Qualifying plans are listed in [Personal insights plans and environments](../../personal/overview/plans-environments.md). If an organization has no *qualifying plan*, its members can't see any of the Viva Insights elements, including the dashboard, and Viva Insights does not use their data.

#### Q2. I used to get a dashboard full of data but now it's grayed out. What happened?

Your personalized data is only accurately calculated when you have a minimum level of activities with Microsoft 365. Check back on your Viva Insights dashboard in a few days to get a fully personalized experience.

#### Q3. Even though I don't have a Viva Insights license, why is the Viva Insights toggle "on" in the dashboard's Feature settings? What will happen if I change it?

Viva Insights will not be available to you if you don't have a license. However, your data contributes to the email read statistics for other users. For example, when you receive a qualifying email and read it, Viva Insights includes that statistic in the read percentage that's shown to the sender. You can change this and not contribute data by turning the Viva Insights toggle off in [Opt out](../use/opt-out-of-mya.md) on your Viva Insights dashboard.

#### Q4. How can I find out what my plan is?

Some Viva Insights feature descriptions start with _**Applies to**_ sections that refer to Microsoft 365 or Office 365 plans and point to [Plans and environments](plans-environments.md) for details.
...
3. Under **Subscriptions**, select **View subscriptions**:
...
#### Q5. In what languages are the elements of Viva Insights available?

See [Viva Insights languages](mya-languages.md).
...
Viva Insights uses Windows 10 activity history to power insights related to time spent in documents within working hours and after hours. These show up as Food for thought in the Viva Insights dashboard. The time spent in documents is also added to the total collaboration time and affects metrics, such as “Time available to focus” and “Quiet hours.”
...
No; currently the Viva Insights service plan cannot be used with shared mailboxes.
...
Turn on the "Viva Insights Outlook add-in" OR "Viva Insights (formerly MyAnalytics)" service plan for the pilot users. Because these plans are enabled by default, you'll need to confirm that the plans are turned off for other users. For details on how to turn off the plans, see [Assign licenses to users](/office365/admin/subscriptions-and-billing/assign-licenses-to-users).
...
See [Browser support in Plans and environments](plans-environments.md#browser-support) for a list of web browsers that the Viva Insights dashboard supports.

As an Outlook add-in, the Viva Insights Outlook add-in requires a browser compatible with your system's platform and operating system. For details, see [Browsers used by Office Add-ins](/office/dev/add-ins/concepts/browsers-used-by-office-web-add-ins).

#### Q2. How can I confirm that the Viva Insights Outlook add-in is installed?
...
The dashboard is available to Viva Insights users a few days after getting assigned a license with a Viva Insights service plan. For more details about when new users get access to Viva Insights, see [Personal insights plans and environments](./plans-environments.md).

#### Q4. How do the Viva Insights digests show up in Outlook?

 The [Viva Insights digests](../use/email-digest-2.md) are system generated notifications that do not go through the standard email delivery process. They are inserted directly into a user's Outlook inbox by Viva Insights. Digests cannot be managed by email setting transport rules.
...
#### Q6. Which Viva Insights features are _not_ available to users who have the "Viva Insights Outlook add-in" service plan?

The cards that show [email read rates and document open rates](../use/use-the-insights.md#track-email-and-document-open-rates) are not currently available with the *Viva Insights Outlook add-in* service plan.

   > [!Note]
   > All Viva Insights features are available to users who have the *Viva Insights (formerly MyAnalytics)* service plan.

#### Q7. Why can't licensed users see one or more of the Viva Insights elements?

* Check [Access to Viva Insights elements](./plans-environments.md#access-to-viva insights-elements) to see when Viva Insights elements become available after users are assigned a license with a Viva Insights service plan.
...
The same rules apply as with Microsoft Microsoft 365 commercial online services, Microsoft personnel do not have access to customer data in Viva Insights. To learn more, see [Who can access your data](https://www.microsoft.com/trust-center/privacy/data-access).
...
Yes, you can delete and restrict the processing of Viva Insights user data if required by law or when requested by a user, which supports GDPR data subject rights. For delete data instructions, see [the third obligation of GDPR Compliance](privacy-guide-users.md#gdpr-compliance).
