...
Posts will typically take up to 1 hour to appear in a user’s Feed.  Content from a newly created [home site](home-site-plan.md) is sometimes delayed for up to seven days after the site is initially created. After that initial 7-day period has passed, it will take the up to 1 hour to appear.
...
In the [Feed for Viva Connections web part](/viva/connections/use-feed-web-part-for-viva-connections), news posts from [organizational news sites](/sharepoint/organization-news-site) up to 7 days from the post date will take precedence over older boosted News that hasn't yet reached the expiration. This can prevent boosted news from displaying prominently to end-users if the boosted news publishing date is older than the organizational news.
