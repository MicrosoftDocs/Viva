...
Connections and SharePoint home sites (also referred to as intranet portals) are two complementary methods to creating powerful employee experiences that can be viewed on the web and in Teams. [Learn more about how Connections and SharePoint home sites work together](/viva/connections/viva-connections-overview#how-sharepoint-home-sites-and-viva-connections-work-together).

A SharePoint home site is not required to set up Viva Connections, but it’s recommended that you add one while setting up Connections to reduce the risk of needing to manually copy content between experiences in some cases. [Learn more about how to plan and build a SharePoint home site](/viva/connections/home-site-plan).

Take the following scenario:  An organization that doesn't have an existing intranet home site creates a Viva Connections experience using the standalone out-of-the-box option during creation. Over a year the organization populates the experience with content.
...
#### To add a SharePoint home site after you've set up a Connections experience

1. Follow the steps outlined for [building from an existing intranet portal](#build-from-an-existing-intranet-portal).
2. If your organization has already set up content in the Connections dashboard and the navigation sections from Teams, you’ll need to manually copy content to the SharePoint home site, otherwise the original content will be hidden in Teams when you publish the SharePoint home site.

   If your organization doesn’t have any custom Connections content in Teams yet, you don't need to manually copy content. Instead, you can **Save** the SharePoint home site and enable the experience.
...
3. For organizations who have already set up dashboard and navigation content in Teams, keep the home site in **Draft mode** until the content has been copied to the SharePoint home site.

#### How to copy content to the SharePoint home site

Follow these instructions if your organization already has custom Connections dashboard and navigation content set up from Teams that you want to preserve when you add a SharePoint home site.

> [!NOTE]
> You'll need SharePoint admin permissions or higher to complete the following steps.

1. Keep the new SharePoint home site in **Draft mode** until content has been copied into the site.
2. Navigate to the SharePoint home site. Next go to **Settings**.
3. Copy dashboard content from Teams to SharePoint. To set up the dashboard from the SharePoint home site, go to **Set up Viva Connections > Create dashboard**. Preview and then **Publish** the dashboard when it’s ready to be shared with others.
...
5. After all the content you want to preserve has been copied to the SharePoint home site, navigate back to the Connections experience in the MAC and turn the **Draft mode** toggle off and then select **Save**.
