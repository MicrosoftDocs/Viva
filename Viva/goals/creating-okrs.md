...
1. Go to the individual, team, or organization that the objective is for. Select the **+** button.
    ...
    ![Screenshot showing where you create an objective.](../media/goals/3/31/a.jpg)

1. In the dialog box that opens:

    1. Enter a short but descriptive name for the objective.
...
![Screenshot showing the key results dialog box.](../media/goals/3/31/c.jpg)
...
![Screenshot showing creating key results.](../media/goals/3/31/d.jpg)
...
![Screenshot showing template options.](../media/goals/3/31/e.jpg)
...
>[!NOTE]
>Key results of company-level objectives will cascade down to act as objectives at the team level only when you've chosen the option **Objectives and key results can be used interchangeably** in the OKR Model Configuration section in the Admin Dashboard.
...
Many companies following the OKR process find it valuable to set annual objectives to keep their broader vision at the forefront and use quarterly objectives to execute the steps toward that vision. 
...
**Each annual objective should have three to five quarterly key results.** These key results should be department or team-level (depending on the size of your company) objectives, maintaining alignment through the cascading hierarchy.
...
**1. First, go to the appropriate annual time period.** By default, Viva Goals features annual time periods in the format **Jan 1-Dec 31 20*XX***. But if your company follows a different cadence, you can set up a custom time period from **Admin** > **Time Periods**.
...
**3. Proceed as normal by setting up team and individual-level quarterly OKRs.** As team members make check-ins, Viva Goals  automatically calculates progress toward your annual objectives. Maintain a bird's-eye view the dashboards. Use views or the objective explorer to drill down and generate reports when necessary.
...
For **committed objectives**, 100-percent completion ensures a score of 1, and the range is marked in green. This indicates that the user has reached the target that they committed to achieve.

For **aspirational objectives**, if a team attains 100 percent of the target and thus closes an objective with a score of 1, the goal wasn't ambitious enough and is marked orange. An ideal aspirational OKR or a stretch goal should be challenging but attainable. A score of 0.7-0.9 is recommended.

## Customize your scoring system in Viva Goals

Viva Goals has a setting so the administrator can override the default ranges for the color schemes provided to signify OKR progress. The administrator can configure the scoring ranges against which the color schemes are mapped.

![Screenshot showing scoring customization.](../media/goals/3/31/f.jpg)

## Copyer or duplicate objectives 

### How to copy or duplicate a single objective 

To copy or duplicate a single objective, follow these steps:

1. Hover over the objective, and select **Clone** from the dropdown on the far right. Or copy from the objective page by selecting **Actions > Clone**. A dialogue box opens.
1. Edit the time period, owner, and option to copy the objective and key results or just the objective.
...
![Screenshot showing where to clone an objective.](../media/goals/3/31/g.jpg)
...
![screenshot showing how to clone multiple objectives.](../media/goals/3/31/h.jpg)
...
![screenshot showing how to change the owner of an O K R.](../media/goals/3/31/i.jpg)

- You can assign [multiple owners](https://help.ally.io/en/articles/4601340-shared-okrs-projects-multiple-teams-owners).
- If there's just one owner or multiple owners assigned to an OKR and you want a user within the organization to check in and update the progress of the OKR, you can use the "check-in responsibility" feature. The "check-in" owner can check in manually or set up an integration to automate this process.
...
If you need to change an OKR or remove one that was made in error, find those options in the menu dropdown menu in the upper-right corner of Viva Goal. Select **Edit** or **Delete** to make the necessary changes.

>[!NOTE]
>An objective can't be edited or deleted if it is **Postponed** or **Closed**. In those cases, the objective must be reopened before you can edit or delete it.

![Screenshot showing how to edit an O K R.](../media/goals/3/31/j.jpg)
...
![Screenshot showing how to print an O K R.](../media/goals/3/31/k.jpg)
