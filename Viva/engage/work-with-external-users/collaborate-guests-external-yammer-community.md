...
There are two methods of working with guests, depending on how your organization’s Viva Engage network is structured. A Viva Engage network [aligned to native mode](../configure-your-yammer-network/overview-native-mode.md) applies the Azure AD-B2B guest framework to power external users while a Viva Engage network that is currently not aligned to native mode uses the Viva Engage-specific guest framework.
