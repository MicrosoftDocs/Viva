...
For the best end-user and management experience, it's required that one Office 365 tenant is associated with just one Yammer network. If you aren't in this configuration, we recommend you [consolidate previously migrated basic networks](consolidate-multiple-networks.md) into a single primary network for your tenant in order to move Yammer into a supported state for your tenant.
...
You can either [consolidate your Yammer networks](consolidate-multiple-networks.md), or change the user's account, as explained in [A Yammer user is displayed as former member when you use Office 365 sign in for Yammer](../troubleshoot-problems/viva-engage-user-is-displayed-as-former-member.md).
