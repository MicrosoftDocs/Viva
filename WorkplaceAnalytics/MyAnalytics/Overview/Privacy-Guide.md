...
This page answers key questions on how MyAnalytics processes information in a manner that protects employee privacy and supports compliance with local regulations like 
...
MyAnalytics processes data from two sources: Exchange Online email and calendar data, and Teams chat and call signals. MyAnalytics stores and processes this data inside each employee’s Exchange Online mailbox.</li>

<li>

**MyAnalytics supports General Data Protection Regulation (GDPR) compliance**. Microsoft has designed MyAnalytics to support customers’ needs to comply with [GDPR requirements](https://www.microsoft.com/trustCenter/privacy/gdpr).</li>
...
1.	**Mailbox data**: Email, calendar, and Teams chat or call activity that people generate by using Office 365, such as time spent in meetings and emails sent to a specific person.
...
These insights are derived from data that is *already available* to people in their Exchange Online mailbox and Teams chat or call history. MyAnalytics simply applies some basic calculations and rules to make that data more actionable. Mailbox data is stored directly in each employee's Exchange Online mailbox.
...
**Determine which people have access to MyAnalytics**. Admins can determine which people can access and use MyAnalytics by issuing licenses to only those people who should have access.</li>
...
1.	**Secure and protect personal data of data subjects**.
    ...
    All MyAnalytics data is stored in the employees’ Exchange Online mailbox. MyAnalytics appends computed metrics such as “Meeting hours” to the mailbox. Thus, MyAnalytics meets this obligation by virtue of Exchange Online also meeting the obligation:
     <ul>
       ...
       <li>Microsoft will not voluntarily disclose Exchange Online customer data to law enforcement agencies</li>
