...
TimeZone |Time zone in which the employee performs work. This must be one of the time zones in Time zones in Workplace Analytics. If you do not have a time zone available for each employee, the system will use the default, which is Pacific Standard Time. | Each row must contain a valid TimeZone and meet valid formatting (see the list of [time zones for Workplace Analytics](../use/Timezones-for-workplace-analytics.md)). Example: “Tokyo Standard Time”.
