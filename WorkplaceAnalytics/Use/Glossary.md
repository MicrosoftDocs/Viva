...
| <a name="meeting-define"></a> Meeting | An audio or video communication or in-person meeting that has been scheduled -- that is, it appears on a person's Outlook calendar. A meeting must involve two or more people. Note that while a meeting's duration is _based on_ its Outlook calendar event, it might not have the _exact same_ duration as that event because Workplace Analytics applies post-processing logic that removes overlapping meetings and applies meeting exclusions. |
