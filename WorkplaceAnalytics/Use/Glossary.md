...
| Hashing | Hashing is a cryptographic process that converts a piece of data into another in a fashion that is easy to compute, extremely difficult to reverse, and highly unlikely that two different pieces have the same hash. For example, meeting [subject lines](https://docs.microsoft.com/en-us/workplace-analytics/use/settings#hash-subject-lines) that are hashed would appear not in their original, readable, form but as a meaningless number. | 
