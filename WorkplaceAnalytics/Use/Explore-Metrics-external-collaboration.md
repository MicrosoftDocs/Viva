...
2. **Measured employees** are those employees who have been assigned a Workplace Analytics license. This represents the number of measured employees in in an organization. (This number does not change with any filter applied in [Page settings](#page-settings).
...
5. This number represents the percentage of measured employees who had one or more external contacts during the selected period. The percentage is the number of measured employees who had at least one external interaction, that is, external collaboration hours greater than zero divided by the total measured employees, multiplied by 100.
6. This number represents external collaboration hours as a percentage of the total collaboration hours spent y measured employees. The percentage is the number of external collaboration hours divided by the number of total collaboration hours multiplied 100.

7. This number represents the total number of external people with whom the measured employees have collaborated at least once during the selected period, that is, external collaboration hours greater than 0. This number does not change with any filter applied in Page settings.

External collaboration features various graphs whose titles suggest different ways to analyze your company's external collaboration activity. Each graph allows you to manipulate data based on settings you select and apply from a menu, such as collaboration hours or collaboration cost.

![Explore filters](../images//WpA/Use/groups-most-time-collab-ext-people.png)

## Trend and Distribution graphs

With some exceptions, the graphs also include Trend and Distribution graphs. Trend graphs show changes over time, for example, the trend of the group average, month by month. 
...
[Page settings](#page-settings), which is common to all sections of Explore, can help you choose the date ranges you want to see for the various charts, or apply filters to a chart’s dataset.
...
This chart shows the trend of the total count of distinct measured employees across all groups of employees (grouped by the investor group-by attribute in page settings).
...
This bar represents the average external network size for the organization, which is the total number of non-distinct external contacts with whom measured employees from this organization had two meaningful interactions in all selected weeks divided by the number of weeks in the selected date range in the page filters on the right.
...
## External domains
The following graph shows you which domains your people spend the most time with. You can choose to pivot the data on various settings, such as total collaboration hours, total collaboration cost, or employee engagement.
...
When this menu option is selected, this bar represents the total number of hours that measured employees have collaborated with sample domain, contoso.com. The bars are sorted on hours (descending).

*Total collaboration cost*

When this menu option is selected, this bar represents the total number of hours that measured employees have collaborated with contoso.com multiplied by the average hourly wage for that tenant (from tenant's configuration). Bars are sorted on cost (descending).

*Total employees engaged*

When this menu option is selected, the bar represents the total number of distinct measured employees who have collaborated with contoso.com.
...
Each bar shows how much time people from the tenant have spent with contoso.com multiplied by the average hourly wage for that tenant (from tenants' configuration) Minimum, maximum, median, upper and lower quartile are shown). Distribution bars are sorted on maximum values descending. There is no distribution to show for the number of employees engaged.
...
Collaboration hours represents email plus meeting hours combined. Each bar represents the average number of collaboration hours that measured employees in that organization have collaborated with external domains, where average is the sum of collaboration hours during all selected weeks divided by the number of weeks in the selected date range in the page filters on the right..
 
The Average line on bar charts represents the average of averages, which is the sum of the average of collaboration hours for each organization divided by the number of organizations. 

Average collaboration hours for each organization represents the total collaboration hours for an organization divided by the number of measured employees in that organization. Bars are sorted on hours (descending.
 
*Trend chart*

Group average is the trend of average of averages for a week, where average of averages is the sum of average collaboration hours for each organization divided by number of organizations.

The average collaboration hours for each organization is the sum of collaboration hours for an organization divided by the number of measured employees in that organization.

*Distribution chart*

Each bar shows how many collaboration hours (minimum, maximum, median, upper and lower quartile) measured employees from each organization have spent with external domains. Distribution bars are sorted on maximum values descending.
