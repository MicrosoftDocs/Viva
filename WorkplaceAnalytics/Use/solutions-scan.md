...
4.	Consider which of the displayed opportunities you would like to pursue. After you’ve decided to start an improvement plan from an opportunity, go on to Create an improvement plan. 

## Create an improvement plan

In the following example walkthrough, we look at the Increase focus opportunity to see how to take advantage of an opportunity to create an improvement plan for a group. 
...
3.	Select See results. This shows a detailed page for the Increase focus opportunity:
 
(The other opportunities follow this exact same model of data presentation.)
4.	Notice the groups that are displayed under Select a group to change the view of your chart. These groups were identified as potentially benefitting from a plan. If a group has appear in this list for multiple weeks, a notification to that effect is displayed:
...
10.	 Workplace Analytics reports whether the group successfully validated, and it also displays any warnings that are generated. For more information about these warnings and about the options upon a validation failure, see Validation. 
11.	After the group validates successfully, go to Start the plan. 
 ...
 * Valid address? Is their email address valid? This check helps verify that the person still works for the company. (An "invalid" email address means that the email address was formatted incorrectly, the person is not a measured employee (they have no Workplace Analytics license or their data could not be processed), or the person is not in the scope that the user is currently working in.
 * Have licenses? Do they have a MyAnalytics license and a Workplace Analytics license? To participate in a plan, they must have both.
 * In a plan? Are they already enrolled in a plan? A person can be in only one plan at a time.
 * Opted out? Have they opted out of MyAnalytics? If the person has opted out, they cannot participate in plans.
...
Three of the possible errors are shown here. If your number of qualified participants (25, in this case) is above the minimum group size, you can move forward with the plan. (You can go right to Start the plan.)
