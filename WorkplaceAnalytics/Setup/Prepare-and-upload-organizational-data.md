...
ManagerId | Unique identifier for the employee’s manager. Can be email address or alias and must be in simplified format (see PersonId for examples.) This data is needed to correctly calculate metrics for time spent with managers and their direct reports. | Each row must contain a valid ManagerId.
 TimeZone |Time zone in which the employee performs work. |  Each row must contain a valid TimeZone and meet valid formatting (see the list of [time zones for Workplace Analytics](../Use/Timezones-for-workplace-analytics.md)). Example: “Tokyo Standard Time” |
...
> [!Important] 
> Numerical fields (such as "HourlyRate") must be in "number" format and cannot contain commas​. Also, the .csv file must use UTF-8 encoding​. For more information about saving a file in UTF-8 format, see [Solution](../Tutorials/download-utf8-query-report.md#solution).
 
### Allowed time zones
The default time zone for Workplace Analytics is Pacific Standard Time. See [Time zones for Workplace Analytics](../use/timezones-for-workplace-analytics.md) for a complete list of times zones that you can use.
