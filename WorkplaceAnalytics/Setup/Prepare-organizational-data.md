...
---------|----------|---------
PersonId |Unique identifier for the employee record. This identifier can be the primary SMTP address, the email alias, or the UPN (if a valid email address) of the employee. This identifier must be in a simplified format that contains no spaces. For example: _person.name@xyz.com_ (not _<Name, Person>_ _(person.name@xyz.com)_). | Each row must contain a valid PersonId. Each upload file can have only ONE record with the same PersonID / EffectiveDate pair.
