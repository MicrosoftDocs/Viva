...
---------|----------|---------
PersonId |Unique identifier for the employee record. Use the primary SMTP address of the employee, in a simplified format with no spaces. For example: <li>Allowed: _person.name@xyz.com_</li><li>Not allowed: _<Name, Person>_ _(person.name@xyz.com)_</li> | Each row must contain a valid PersonId. Each upload file can have only ONE record with the same PersonID / EffectiveDate pair.
