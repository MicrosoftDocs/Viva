...
HourlyRate | The employee’s salary represented as an hourly rate in US dollars. Notes:<br><li>If the HR upload provides an annual rate, you’ll need to divide each record by 2080.</li><li>The value can be formatted as a whole number, or include two decimal places, and cannot include any special characters, such as a dollar sign.</li><li>The value can represent pay only, or include the full value of benefits, as long as that choice is consistently applied for all employees.</li><li>This rate is used in calculations and can be used to filter and group employees.</li><li>If the upload doesn’t include a salary or hourly rate for an employee, Workplace Analytics uses a default HourlyRate of $75.</li><li>You can change the default rate in [Admin settings](../use/settings.md#admin-settings). If you change the default, the change applies retroactively to anyone without an effective hourly rate (or salary) for the next scheduled (usually set monthly) refresh of your organizational (HR) data.|This attribute column is not required. If it is included, then each row must contain a floating point or integer value with no special characters (such as a dollar sign).|
