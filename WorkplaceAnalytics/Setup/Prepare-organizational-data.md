...
HourlyRate | The salary of the employee, represented as an hourly rate in US dollars (if you have an annual rate, divide each record by 2080). This attribute is used in calculations and can be used to filter and group employees.<p><p>**Notes:** The value can be formatted as a whole number or include two decimal places. It cannot include any special characters, such as a dollar sign.<p>The value can represent pay only, or can include the full value of benefits, as long as that choice is consistently applied for all employees.<p>When the default rate is changed (see [Admin settings](../use/settings.md#admin-settings)), this change applies retroactively to anyone without an effective hourly rate for the next scheduled (usually set monthly) refresh of your organizational (HR) data.<p>Note that the Explore-the-metrics feature uses a fixed default HourlyRate of $75.  |This attribute column is not required. If it is included, then each row must contain a floating point or integer value with no special characters (such as a dollar sign).|
