...
SupervisorIndicator | Identifies each employee with the designations **IC** (individual contributor), **Mngr** (manager), or **Mngr+** (manager of managers). (However, note that if different nomenclature is used in your file, you will need to update the Power BI chart filters accordingly.) This attribute is required to view the habits of people managers in your organization. It powers the Overview table as well as the Generated Workload charts that are generated when you use the Manager Impact Power BI template.  | This attribute column is required to use the Manager Impact dashboard to the fullest extent. |
