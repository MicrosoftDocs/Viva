...
| <a name="groupid-definition"></a> GroupId | A unique, de-identified ID number that identifies a group. It is the same for all members of a team who report to another person in the organizational hierarchy. A GroupId column can be used to aggregate members of a team based on the [organizational hierarchy data that's uploaded](../setup/upload-organizational-data-1st) by a Workplace Analytics admin.  | No coverage requirements. |
