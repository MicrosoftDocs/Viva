...
SupervisorIndicator | Use this attribute to view the habits of people managers in your organization in Power BI visualizations. It powers the Overview table as well as the Generated Workload charts that are generated when you use the Manager Impact [Power BI template](../tutorials/power-bi-templates.md).<p></p>SupervisorIndicator identifies each employee with the designation **IC** (individual contributor), **Mngr** (manager), or **Mngr+** (manager of managers); however, note that if different nomenclature is used in your file, you must update the Power BI chart filters accordingly. | This attribute column is required to use the Manager Impact dashboard to the fullest extent. |
