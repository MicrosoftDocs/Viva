...
The data that Workplace Analytics uses is refreshed once a week, on Sunday. This includes data records for licensed users. If you add a user license on Tuesday, that person will not show up in Workplace Analytics until the data refresh the following Sunday. After the data refresh, the newly licensed user appears in Workplace Analytics in the following ways:
 ...
 * [Queries](../tutorials/query-basics.md) that analysts run can return results that include data about these users' activities. 
 * The [Explore](../use/explore-intro.md) dashboards can reflect data that includes the newly licensed employees. 
